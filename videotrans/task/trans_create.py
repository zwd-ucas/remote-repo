...
class TransCreate(BaseTask):
    ...
    def _parse_inline_voice_tags(self, subs) -> dict:
        """解析字幕每行开头的 [角色-音色] / 【角色-音色】 标记，自动逐行分配音色。

        返回 {行号(str): 菜单中的音色完整名}。命中已知音色的行会就地去掉行首标记，
        并把去标记后的干净字幕回写 target_sub（让朗读与烧录/显示都不含标记）。
        未命中任何已知音色的方括号原样保留，不影响其它字幕或正常方括号文本。
        """
        try:
            menu = tools.role_menu(self.cfg.tts_type, None) or []
        except Exception:
            menu = []
        # 以“括号前中文名”为键，映射到菜单完整名（传给 TTS 用，如 沧明子 -> 沧明子(Eldric Sage)）
        name2full = {}
        for v in menu:
            if not isinstance(v, str) or v in ('No', 'clone'):
                continue
            name2full.setdefault(v.split('(')[0].strip(), v)
        if not name2full:
            return {}
        ...
        changed = False
        for it in subs:
            m = re.match(r'^\s*[\[【]\s*([^\]】]+?)\s*[\]】]\s*', it['text'])
            if not m:
                continue
            tag = m.group(1).strip()
            # 取能作为标记后缀的最长音色中文名（兼容含连字符的方言名，如 上海-阿珍）
            hit = None
            for name in name2full:
                if tag == name or tag.endswith('-' + name) or tag.endswith(name):
                    if hit is None or len(name) > len(hit):
                        hit = name
            if not hit:
                continue
            inline_roles[f'{it["line"]}'] = name2full[hit]
            it['text'] = it['text'][m.end():].strip()
            changed = True
        ...
        if changed:
            try:
                ...
                Path(self.cfg.target_sub).write_text(clean, encoding='utf-8')
                logger.debug(f'[行内音色标记] 命中 {len(inline_roles)} 行，已回写去标记字幕')
            except Exception as e:
                logger.exception(f'[行内音色标记] 回写去标记字幕失败: {e}', exc_info=True)
        return inline_roles
    ...
    def _tts(self) -> None:
        ...
        # 自定义增强：解析字幕行内 [角色-音色] 标记，得到 {行号: 音色}，并清理标记文本
        inline_roles = self._parse_inline_voice_tags(subs)

        # 取出每一条字幕，行号\n开始时间 --> 结束时间\n内容
        for i, it in enumerate(subs):
            if it['end_time'] < it['start_time'] or not it['text'].strip():
                continue
            # 判断音色优先级：字幕行内标记 > 弹窗逐行设置(line_roles) > 全局音色
            voice = 'clone' if force_clone else (
                inline_roles.get(f'{it["line"]}') or line_roles.get(f'{it["line"]}', voice_role))
